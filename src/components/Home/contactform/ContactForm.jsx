...
const ContactForm = ({popup}) => {
    ...
    return (
      <div className="px-3 xl:px-[120px] py-10 w-full flex flex-col gap-2 lg:gap-7">
        <h1 className={`font-semibold gradient-heading ${popup ? "text-xl": "text-[40px] "}`}>
          Contact Us
        </h1>
        ...
        <button onClick={()=>handleEnquiryFormClick({firstName, lastName,email,phone,message})} className="text-base font-semibold bg-[#1ECBAF] text-white p-2 hover:bg-btnHover nav-link rounded-lg">
          Send Message
        </button>
      </div>
    );
